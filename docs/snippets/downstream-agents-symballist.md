...
- Check freshness first:
  - `bun run D:\Projects\symballist\src\cli.ts status --root <PROJECT_ROOT>`
- If the index is stale, refresh it before relying on results:
  - `bun run D:\Projects\symballist\src\cli.ts index --root <PROJECT_ROOT>`
- Use query for discovery:
  - `bun run D:\Projects\symballist\src\cli.ts query "<text>" --root <PROJECT_ROOT>`
- Use show for full context and related symbols:
  - `bun run D:\Projects\symballist\src\cli.ts show <id> --root <PROJECT_ROOT>`

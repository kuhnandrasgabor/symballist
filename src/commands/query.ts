...
export async function runQuery(
  ...
  const payload = {
    ...
    ...(options.compact === true ? {} : {
      ...
      graphDiagnostics: "graphDiagnostics are index-bounded structural signals for each result, such as no known inbound references, test-only inbound references, same-file-only connectivity, disconnected-from-indexed-graph, and root-like status. They are not dead-code claims.",
